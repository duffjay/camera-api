...
def is_frame_different(consumer_id, camera_id, np_images):
    ...
    log.debug(f'  IMAGE-CONSUMER:<< {consumer_id} cam: {camera_id} qsize: {settings.imageQueue.qsize()}'
            ...
            if image_different == True:
                ...
                for region_id, np_image in enumerate(np_images):
                    ...
                    with settings.safe_print:
                        log.info(
                            f'  IMAGE-CONSUMER:<<{camera_id:02}-{consumer_id:03} qsize: {settings.imageQueue.qsize():03}'
                            f'  diff: {score:0.3f}/{threshold:0.3f} reg: {region_id} timestamp {image_time}'
                            f'  infQtime:{(time.perf_counter() - start):02.2f} sec  dets: {inf.detection_count} new: {new_objects}  saved: {saved}'
                        )
                        if inf.detection_count > 0:
                ...
                with settings.safe_print:
                    log.info(f'  IMAGE-CONSUMER:<<{camera_id:02}-{consumer_id:03} No difference in image, diff score: {score:0.3f} adj thresh:{threshold:0.3f}')
